...
public interface ProductRepository extends JpaRepository<Product, Integer> {
    ...
    // ✅ Tìm kiếm sản phẩm theo tên + ngôn ngữ
    @Query(value = """
        SELECT 
            p.product_id,
            pt.product_name,
        ...
        LEFT JOIN product_category_translation pct
            ON pc.product_category_id = pct.product_category_id AND pct.language_id = :lang
        WHERE 
            (:keyword IS NULL OR LOWER(pt.product_name) LIKE CONCAT('%', LOWER(:keyword), '%'))
        """, nativeQuery = true)
    List<Object[]> searchProductsByLanguage(@Param("keyword") String keyword,
